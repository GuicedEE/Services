 ...
 */
package org.hibernate.reactive.pool.up.impl;
...
public class DefaultSqlClientPool extends SqlClientPool
    ...
    private static final Logger log = Logger.getLogger( DefaultSqlClientPool.class.getCanonicalName() );
    ...
    protected Pool createPool(URI uri, SqlConnectOptions connectOptions, PoolOptions poolOptions, Vertx vertx) {
        try {
            ...
            return driver.createPool( vertx,()->{
               return Future.succeededFuture();
            } ,poolOptions,new NetClientOptions(),connection->{
    ...
    public static URI parse(String url) {
        if ( url == null || url.isBlank() ) {
            throw new RuntimeException(Settings.URL );
        }
