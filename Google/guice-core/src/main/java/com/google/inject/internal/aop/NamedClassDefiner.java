...
/**
 * {@link ClassDefiner} that defines named classes using {@code MethodHandles.Lookup#defineClass}.
 * Classes defined this way are normal named classes in the host class's package and class loader.
 *
 * @author mcculls@gmail.com (Stuart McCulloch)
 */
final class NamedClassDefiner implements ClassDefiner {

  @Override
  public Class<?> define(Class<?> hostClass, byte[] bytecode) throws Exception {
    try {
      Module guiceModule = NamedClassDefiner.class.getModule();
      ...
      // addOpens/addReads may throw IllegalCallerException if called from outside the target
      // module. In that case we rely on privateLookupIn succeeding (e.g. via --add-opens flags)
      // or the fallback lookup strategies below.
    }
