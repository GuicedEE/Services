...
public final class RealMultibinder<T> implements Module {
  ...
  @SuppressWarnings("unchecked")
  static <T> TypeLiteral<Collection<Provider<T>>> collectionOfJakartaProvidersOf(
      TypeLiteral<T> elementType) {
    Type providerType =
        Types.newParameterizedType(Provider.class, elementType.getType());
    Type type = Types.collectionOf(providerType);
    return (TypeLiteral<Collection<Provider<T>>>) TypeLiteral.get(type);
  }
  ...
  private static final class BindingSelection<T> {
    ...
    private Key<Collection<Provider<T>>> collectionOfJakartaProvidersKey;
    ...
    Key<Collection<Provider<T>>> getCollectionOfJakartaProvidersKey() {
      Key<Collection<Provider<T>>> local = collectionOfJakartaProvidersKey;
      if (local == null) {
        local =
